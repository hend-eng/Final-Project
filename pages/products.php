...
/**
 * Compute the project's root URL path dynamically, based on where this
 * script actually lives on the server. This means the site will keep
 * working no matter what the project folder is named or where it's
 * placed (e.g. /Final-Project/, /shop/, /htdocs/whatever/ ...).
 *
 * This file lives at:  <root>/pages/products.php
 * So going up one level from SCRIPT_NAME's directory gives us <root>.
 */
...
function getProductColors(array $product): array
{
    ...
    if (is_array($colors)) {
        return array_values(array_filter(array_map('trim', array_map('strval', $colors))));
    }
    ...
    if (is_string($colors)) {
        ...
        return array_values(array_filter(array_map('trim', explode(',', $colors))));
    }
...
function getProductSizes(array $product): array
{
    ...
    if (is_array($sizes)) {
        return array_values(array_filter(array_map('trim', array_map('strval', $sizes))));
    }
...
{
    $key = strtolower(trim(preg_replace('/\s+/', ' ', $color)));
...
function buildPageUrl(int $page, array $filters, string $sort): string
{
    $query = $filters;
    $query['sort'] = $sort;
    $query['page'] = $page;
    ...
    <section class="category-header">
        <div class="container">
            ...
            <div class="category-breadcrumb">
                <a href="<?= htmlspecialchars($siteRoot, ENT_QUOTES, 'UTF-8') ?>/index.php">Home</a>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3">
                ...
                <div>
                    ...
                    <p class="mb-0">
                        <?= count($filtered) ?>
                        product<?= count($filtered) === 1 ? '' : 's' ?> found
                    </p>
                </div>
                ...
                <aside class="col-12 col-md-3">
                    <div class="filters" id="filters-panel">
                        ...
                        <form method="get" id="product-filter-form">
                            <input type="hidden" name="category" id="filter-category" value="<?= htmlspecialchars($category, ENT_QUOTES, 'UTF-8') ?>">
                            <input type="hidden" name="color" id="filter-color" value="<?= htmlspecialchars($color, ENT_QUOTES, 'UTF-8') ?>">
                            <input type="hidden" name="size" id="filter-size" value="<?= htmlspecialchars($size, ENT_QUOTES, 'UTF-8') ?>">
                            <input type="hidden" name="style" id="filter-style" value="<?= htmlspecialchars($style, ENT_QUOTES, 'UTF-8') ?>">
...
<div class="filter-group">
                                ...
                                <div class="filter-list">
                                    ...
                                    <?php foreach ($categories as $name => $_): ?>
                                        <button
                                            ...
                                            data-filter="category"
                                            data-value="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?>"
                                        >
                                ...
                                <div class="color-options">
                                    ...
                                    <?php foreach ($colors as $name => $_): ?>
                                        <button
                                            ...
                                            data-filter="color"
                                            data-value="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?>"
                                            style="background-color: <?= htmlspecialchars(getColorHex($name), ENT_QUOTES, 'UTF-8') ?>;"
                            ...
                            <a
                                href="<?= htmlspecialchars($siteRoot, ENT_QUOTES, 'UTF-8') ?>/pages/products.php"
                ...
                <div class="col-12 col-md-9">
                    ...
                    >
                        ...
                        <?php foreach ($pageProducts as $product): ?>
                            ...
                            $imagePath = $siteRoot . '/' . ltrim($image, '/');
                            ?>
                            ...
                            <div class="col">
                                ...
                                <article class="product-card">
                                    ...
                                    <a
                                        href="<?= htmlspecialchars($siteRoot, ENT_QUOTES, 'UTF-8') ?>/pages/product-details.php?id=<?= urlencode((string) $product['id']) ?>"
                    ...
                    <?php if ($totalPages > 1): ?>
                        ...
                        >
                            ...
                            <?php if ($currentPage > 1): ?>
                                ...
                                <a
                                    href="<?= htmlspecialchars(buildPageUrl($currentPage - 1, $currentFilters, $sort), ENT_QUOTES, 'UTF-8') ?>"
                                    class="prev"
                                >
                            ...
                            <div class="pagination-numbers">
                                ...
                                <?php for ($page = 1; $page <= $totalPages; $page++): ?>
                                    ...
                                    <a
                                        href="<?= htmlspecialchars(buildPageUrl($page, $currentFilters, $sort), ENT_QUOTES, 'UTF-8') ?>"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
        document.querySelectorAll('[data-filter]').forEach(function (button) {
            button.addEventListener('click', function () {
                ...
                document.querySelectorAll('[data-filter="' + filterName + '"]').forEach(function (item) {
                    item.classList.toggle('selected', item === button);
                });

                if (filterName === 'color') {
                    const clearButton = document.querySelector('.clear-option[data-filter="color"]');

                    if (clearButton) {
                        clearButton.style.display = value === '' ? 'none' : 'inline-block';
                    }
                }
            });
        });
    ...
    if (openButton && closeButton && filters) {
