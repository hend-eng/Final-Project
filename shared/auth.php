...
function currentUser(): ?array
{
    ...
    $stmt = $pdo->prepare('SELECT id, full_name, email, role, created_at FROM users WHERE id = ? LIMIT 1');
...
function authRedirectPath(array $user, string $basePath): string
{
    return $user['role'] === 'admin'
        ? $basePath . '/dasboard/index.php'
        : $basePath . '/pages/profile.php';
}
...
/** Block access unless logged in; bounces to login and back afterwards. */
function requireLogin(string $basePath): void
{
    if (!isLoggedIn()) {
        $redirect = urlencode($_SERVER['REQUEST_URI'] ?? '');
        redirectTo($basePath . '/auth/login.php?redirect=' . $redirect);
    }
}
...
/* ---------------------------------------------------------------- *
 * One-time flash messages (survive a single redirect)
 * ---------------------------------------------------------------- */
